...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['nuevo_producto'])) {
    ...
    if ($nombre && $precio > 0) {
        $model->guardar($nombre, $stock, $precio);
...
<head>
    <meta charset="UTF-8">
    <title>Productos - Juguería Aitana</title>
    ...
    <style>
        ...
        .modal-header h3 { font-size: 18px; font-weight: 600; color: #0f3d22; margin: 0 0 2px; }
...
<div class="content">
    ...
    <div class="card">
        <div class="card-header">
            <p class="card-title">Lista de productos</p>
            <button onclick="abrirModal()" class="btn btn-primary btn-sm">+ Nuevo Producto</button>
        </div>
        <div class="table-wrapper">
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <a href="editar.php?id=<?= $p['id'] ?>" class="btn btn-secondary btn-sm">Editar</a>
                    </td>
                </tr>
                <?php endwhile; ?>
                </tbody>
...
<div class="modal-overlay <?php echo ($modalError || $modalExito) ? 'open' : ''; ?>" id="modalOverlay">
    <div class="modal">
        ...
        <?php if ($modalExito): ?>
            <div class="modal-alert exito">✅ <?= htmlspecialchars($modalExito) ?></div>
        <?php endif; ?>
        <?php if ($modalError): ?>
            <div class="modal-alert error">⚠️ <?= htmlspecialchars($modalError) ?></div>
        <?php endif; ?>
...
<script>
    ...
    <?php if ($modalExito): ?>
        document.getElementById('modalOverlay').classList.add('open');
        setTimeout(() => { cerrarModal(); location.reload(); }, 2500);
    <?php endif; ?>
</script>
